...
pub trait CheckOutput<Input>: Sized {
    ...
    fn validate<'a>(self, expected: Self, input: Input, allowed_ulp: u32) -> anyhow::Result<()>;
}
    ...
    u32: TryFrom<F::SignedInt, Error: fmt::Debug>,
{
    fn validate<'a>(self, expected: Self, input: Input, allowed_ulp: u32) -> anyhow::Result<()> {
        ...
        let inner = || -> anyhow::Result<()> {
            ...
            let ulp_diff = act_bits
                .checked_sub(exp_bits)
                .with_context(|| format!("ULP calculation failed: {act_bits} - {exp_bits}"))?
                .abs();
